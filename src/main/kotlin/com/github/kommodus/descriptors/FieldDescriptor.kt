...
sealed class FieldDescriptor<T, A> {
    fun satisfies(constraint: Validation.Constraint<A>): Terminal<T, A> =
        satisfies(constraint as Validation.Validator<A>)

    internal abstract fun satisfies(constraint: Validation.Validator<A>): Terminal<T, A>

    class Opened<T, A> internal constructor(
        private val property: KProperty1<T, A>,
        private val container: ValidationDescriptor<T>
    ): FieldDescriptor<T, A>() {
        override fun satisfies(constraint: Validation.Validator<A>): Terminal<T, A> =
    ...
    class Terminal<T, A> internal constructor(
        private val property: KProperty1<T, A>,
        private val container: ValidationDescriptor<T>,
        private val constraints: List<Validation.Validator<A>>
